...
class JSDocAnalyzer {
		...
		if (targetFile) {
			...
			if (!files.includes(targetPath)) {
				...
				Process.stdout.write(
					`  - For schema files: schema/Schema.ts, schema/AST.ts, etc.\n`,
				);
				Process.stdout.write(
					`  - For config files: config/Config.ts, config/ConfigError.ts, etc.\n\n`,
				);
	...
	generateFileReport(fileStats) {
		...
		Process.stdout.write(
			`         ${filename.toUpperCase()} DOCUMENTATION REPORT\n`,
		);
		Process.stdout.write("=".repeat(60) + "\n\n");
		...
		if (missingExamples > 0) {
			...
			missingExampleItems.forEach((item, index) => {
				Process.stdout.write(
					`${index + 1}. ${item.name} (${item.type}) - Line ${item.line}\n`,
				);
			});
			Process.stdout.write("\n");
		}
	...
	generateReport() {
		...
		const sortedFiles = fileDetails
			...
			.sort(
				(a, b) =>
					b.missingExamples +
					b.missingCategories -
					(a.missingExamples + a.missingCategories),
			)
			.slice(0, 15);
		...
		const perfectFiles = fileDetails.filter(
			(f) => f.missingExamples === 0 && f.missingCategories === 0,
		);
		if (perfectFiles.length > 0) {
			...
			perfectFiles.forEach((file) => {
				Process.stdout.write(
					`   ${file.filename} (${file.totalExports} exports)\n`,
				);
			});
			Process.stdout.write("\n");
		}
		...
		if (sortedFiles.length > 0) {
			...
			const topFileMissingItems = missingItems
				.filter((item) => item.file === topFile.filename)
				.slice(0, 10);
			if (topFileMissingItems.length > 0) {
				Process.stdout.write(
					`🔍 SAMPLE MISSING ITEMS FROM ${topFile.filename}\n`,
				);
				Process.stdout.write("-".repeat(35) + "\n");
				topFileMissingItems.forEach((item) => {
					...
					Process.stdout.write(
						`   ${item.name} (${item.type}, line ${item.line}): missing ${missing.join(", ")}\n`,
					);
				});
				Process.stdout.write("\n");
			}
		...
		Process.stdout.write(
			`Examples: ${documentedExamples}/${totalExports} (${(
				(documentedExamples / totalExports) * 100
			).toFixed(1)}% complete)\n`,
		);
		Process.stdout.write(
			`Categories: ${documentedCategories}/${totalExports} (${(
				(documentedCategories / totalExports) * 100
			).toFixed(1)}% complete)\n`,
		);
		Process.stdout.write("\n");
		...
		Process.stdout.write("=".repeat(60) + "\n");
		Process.stdout.write(
			`Analysis complete! ${missingExamples + missingCategories} items need attention.\n`,
		);
		Process.stdout.write("=".repeat(60) + "\n");
