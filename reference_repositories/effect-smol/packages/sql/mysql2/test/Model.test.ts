		...
		() =>
			Effect.gen(function* () {
				...
				const result = yield* repo.insert(
					User.insert.makeUnsafe({ name: "Alice", age: 30 }),
				);
	...
	it.live(
		...
		() =>
			Effect.gen(function* () {
				...
				const john = yield* repo.insert(
					User.insert.makeUnsafe({ name: "John", age: 30 }),
				);

				const [alice2, john2] = yield* Effect.all(
					[repo.findById(alice.id), repo.findById(john.id)],
					{ concurrency: "unbounded" },
				);
		...
		() =>
			Effect.gen(function* () {
				...
				result = yield* repo.update(
					User.update.makeUnsafe({ ...result, name: "Bob" }),
				);
				assert.deepStrictEqual(
					result,
