			...
			return identity<SqliteConnection>({
				...
				export: Effect.try({
					try: () => db.serialize(),
						...
						new SqlError({ cause, message: "Failed to export database" }),
				}),
				backup(destination) {
					return Effect.tryPromise({
						try: () => db.backup(destination),
						catch: (cause) =>
							new SqlError({ cause, message: "Failed to backup database" }),
					});
				},
				loadExtension(path) {
					return Effect.try({
						try: () => db.loadExtension(path),
							...
							new SqlError({ cause, message: "Failed to load extension" }),
					});
				},
			});
		});
