			...
			yield* Effect.acquireRelease(
				Effect.tryPromise({
					try: () => pool.query("SELECT 1"),
						...
						new SqlError({ cause, message: "PgClient: Failed to connect" }),
				}),
...
export const fromPool = Effect.fnUntraced(function* (options: {
		...
		private runWithClient<A>(
			...
			return Effect.callback<A, SqlError>((resume) => {
				...
				function onError(cause: Error) {
					cleanup(cause);
					resume(
						Effect.fail(new SqlError({ cause, message: "Connection error" })),
					);
				}
	...
	const reserve = Effect.map(
		reserveRaw,
		(client) => new ConnectionImpl(client),
	);
		...
		}),
		{
					...
					Effect.fnUntraced(function* (queue) {
						...
						yield* Effect.tryPromise({
							try: () => client.query(`LISTEN ${Pg.escapeIdentifier(channel)}`),
								...
								new SqlError({ cause, message: "Failed to listen" }),
						});
			...
			notify: (channel: string, payload: string) =>
				Effect.callback<void, SqlError>((resume) => {
					pool.query(
						`NOTIFY ${Pg.escapeIdentifier(channel)}, $1`,
						[payload],
						(err) => {
							if (err) {
								resume(
									Effect.fail(
										new SqlError({ cause: err, message: "Failed to notify" }),
									),
								);
							} else {
								resume(Effect.void);
