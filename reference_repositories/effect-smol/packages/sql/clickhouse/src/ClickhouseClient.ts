 ...
 * @since 1.0.0
 */
export const ClickhouseClient = ServiceMap.Service<ClickhouseClient>(
				...
				return this.runRaw(sql, params, "JSONEachRow").pipe(
					Effect.map((result) => {
						...
						>({
							...
							onError: (cause) =>
								new SqlError({ cause, message: "Failed to execute stream" }),
						});
					}),
					Stream.unwrap,
					Stream.chunks,
					Stream.mapEffect((chunk) => {
						...
						return Effect.tryPromise({
							try: () =>
								...
								new SqlError({ cause, message: "Failed to parse row" }),
						});
					}),
 ...
 * @since 1.0.0
 */
export const ClientMethod = ServiceMap.Reference<
