 ...
 * @since 1.0.0
 */
export const MssqlClient = ServiceMap.Service<MssqlClient>(
	"@effect/sql-mssql/MssqlClient",
);
...
const TransactionConnection =
	Client.TransactionConnection as unknown as ServiceMap.Service<
...
export const make = (
	...
	Effect.gen(function* () {
			...
			yield* Effect.callback<void, SqlError>((resume) => {
				conn.connect((cause) => {
					if (cause) {
						resume(
							Effect.fail(
								new SqlError({ cause, message: "Failed to connect" }),
							),
						);
					} else {
						resume(Effect.void);
		...
		yield* Pool.get(pool).pipe(
			...
			Effect.mapError(
				({ cause }) =>
					new SqlError({ cause, message: "MssqlClient: Failed to connect" }),
			),
			Effect.scoped,
