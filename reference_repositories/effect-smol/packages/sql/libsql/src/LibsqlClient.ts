 ...
 * @since 1.0.0
 */
export const LibsqlClient = ServiceMap.Service<LibsqlClient>(
	"@effect/sql-libsql/LibsqlClient",
);

const LibsqlTransaction = ServiceMap.Service<
				...
				return Effect.map(
					Effect.tryPromise({
						try: () => (this.sdk as Libsql.Client).transaction("write"),
							...
							new SqlError({ cause, message: "Failed to begin transaction" }),
					}),
			...
			get commit() {
				return Effect.tryPromise({
					try: () => (this.sdk as Libsql.Transaction).commit(),
						...
						new SqlError({ cause, message: "Failed to commit transaction" }),
				});
			}
			get rollback() {
				return Effect.tryPromise({
					try: () => (this.sdk as Libsql.Transaction).rollback(),
						...
						new SqlError({ cause, message: "Failed to rollback transaction" }),
				});
			}
		}
