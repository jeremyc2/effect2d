...
describe("HttpApi", () => {
	...
	describe("middleware", () => {
		...
		it.effect("layerClient supports effectful construction", () => {
			class HeaderValue extends ServiceMap.Service<HeaderValue, string>()(
				"HeaderValue",
			) {}
		...
		it.effect("security middleware can be implemented with layerClient", () => {
			class CurrentToken extends ServiceMap.Service<CurrentToken, string>()(
				"CurrentToken",
			) {}
			...
			const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
				handlers.handle("a", () => CurrentToken.asEffect()),
			);
		...
		it.effect("no content", () => {
			...
			const GroupLive = HttpApiBuilder.group(Api, "group", (handlers) =>
				handlers
					...
					.handle("b", () =>
						Effect.succeed(HttpApiSchema.NoContent.makeUnsafe()),
					)
				...
				Layer.provide(
					HttpApiBuilder.group(Api, "group", (handlers) =>
						handlers.handle("error", () =>
							new RateLimitError({ message: "Rate limit exceeded" }).asEffect(),
						),
					),
				),
				HttpRouter.serve,
