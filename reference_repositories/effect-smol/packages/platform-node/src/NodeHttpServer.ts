...
> => {
	...
	return Effect.withFiber((parent) => {
		...
		return Effect.succeed(function handler(
			...
			let nodeResponse_: Http.ServerResponse | undefined;
			const nodeResponse = () => {
			...
			const fiber = Fiber.runIn(
				Effect.runForkWith(ServiceMap.makeUnsafe<any>(map))(handledApp),
				options.scope,
			);
			socket.on("close", () => {
...
> =>
	Layer.mergeAll(
		Layer.effect(HttpServer.HttpServer)(
			Effect.flatMap(Config.unwrap(options).asEffect(), (options) =>
