...
export interface Service {
	...
	readonly createEmbedding: (
		...
		typeof Generated.CreateEmbeddingResponse.Type,
		AiError.AiError
	>;
}
...
export const make = Effect.fnUntraced(
	function* (
		...
		> =>
			client
				.createResponse({ payload, config: { includeResponse: true } })
				.pipe(
					Effect.catchTags({
 ...
 */
export class OpenAiSocket extends ServiceMap.Service<
	OpenAiSocket,
	{
		/**
		 * Create a streaming response using the OpenAI responses endpoint.
...
const makeSocket = Effect.gen(function* () {
	...
	const request = yield* Effect.orDie(
		client.client.httpClient.preprocess(HttpClientRequest.post("/responses")),
	);
	...
	const queueRef = yield* RcRef.make({
		...
		acquire: Effect.gen(function* () {
			...
			let currentQueue: Queue.Enqueue<
				ResponseStreamEvent,
			...
			const cancel = Effect.suspend(() =>
				write(JSON.stringify({ type: "response.cancel" })),
			).pipe(Effect.ignore);
			const reset = () => {
				currentQueue = null;
			};
