	...
	export interface TextPartOptions extends ProviderOptions {
		readonly openai?: {
			...
			readonly annotations?: ReadonlyArray<
				typeof Generated.Annotation.Encoded
			> | null;
		} | null;
	}
}
		...
		switch (message.role) {
			...
			case "assistant": {
				const reasoningMessages: Record<
					string,
					DeepMutable<typeof Generated.ReasoningItem.Encoded>
				> = {};
							...
							if (Predicate.isNotNull(id)) {
								...
								if (config.store === true) {
									...
									const summaryParts: Array<
										typeof Generated.SummaryTextContent.Encoded
									> = [];
...
> {
	...
	parts.push({
		...
		...(rawResponse.service_tier && {
			metadata: { openai: { serviceTier: rawResponse.service_tier } },
		}),
	});
			...
			Effect.fnUntraced(function* (event) {
				...
				switch (event.type) {
					...
					case "response.failed": {
						parts.push({
							...
							...(event.response.service_tier && {
								metadata: {
									openai: { serviceTier: event.response.service_tier },
								},
							}),
						});
						break;
					...
					case "response.output_text.annotation.added": {
						const annotation =
							event.annotation as typeof Generated.Annotation.Encoded;
...
type OpenAiToolChoice =
	(typeof Generated.CreateResponse.Encoded)["tool_choice"];
...
): (typeof Generated.Message.Encoded)["status"] | null =>
...
const makeItemIdMetadata = (itemId: string | undefined) =>
	Predicate.isNotUndefined(itemId) ? { itemId } : undefined;
...
const getUsage = (
	usage: Generated.ResponseUsage | null | undefined,
): Response.Usage => {
