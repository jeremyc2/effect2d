				...
				const fiber = yield* Effect.forkScoped(
					OpenAiClient.withWebSocketMode(
						client
							...
							.pipe(Effect.andThen(([_, stream]) => Stream.runDrain(stream))),
					),
					{ startImmediately: true },
				);

				const createEvent = yield* Effect.promise(
					() => server.nextMessage as Promise<any>,
				);
				assert.strictEqual(createEvent.type, "response.create");

				yield* Fiber.interrupt(fiber);

				const cancelEvent = yield* Effect.promise(
					() => server.nextMessage as Promise<any>,
				);
