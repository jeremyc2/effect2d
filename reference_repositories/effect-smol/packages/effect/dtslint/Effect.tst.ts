...
class AcquireReleaseDependency extends ServiceMap.Service<
...
describe("Effect.services", () => {
	it("defaults R to never", () => {
		...
		expect(result).type.toBe<
			Effect.Effect<ServiceMap.ServiceMap<never>, never, never>
		>();
	});
});
...
describe("Effect.fn", () => {
	...
	it("with a span, function", () => {
		const fn = Effect.fn("span")(() => Effect.fail("bye"));
		expect(fn).type.toBe<() => Effect.Effect<never, string, never>>();
	});
	...
	it("without a span, function", () => {
		const fn = Effect.fn(() => Effect.fail("bye"));
		expect(fn).type.toBe<() => Effect.Effect<never, string, never>>();
	});
	...
	it("should not unwrap nested effects", () => {
		const fn = Effect.fn(() => Effect.succeed(Effect.succeed(1)));
		expect(fn).type.toBe<
