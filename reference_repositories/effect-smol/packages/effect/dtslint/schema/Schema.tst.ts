...
describe("Schema", () => {
	...
	describe("makeUnsafe", () => {
		...
		it("brand", () => {
			...
			expect(schema.makeUnsafe).type.toBe<
				MakeUnsafe<string, string & Brand.Brand<"a">>
			>();
		...
		it("refine", () => {
			...
			expect(schema.makeUnsafe).type.toBe<
				MakeUnsafe<Option.Option<string>, Option.Some<string>>
			>();
		});
		...
		describe("Struct", () => {
			...
			it("nested defaulted & branded field", () => {
				...
				const schema = Schema.Struct({
					a: A.pipe(
						Schema.withConstructorDefault(() => Option.some(A.makeUnsafe({}))),
					),
				});
				expect(schema.makeUnsafe).type.toBe<
					MakeUnsafe<
			...
			it("Class field", () => {
				...
				expect(schema.makeUnsafe).type.toBe<
					MakeUnsafe<{ readonly a: A }, { readonly a: A }>
				>();
			});
		...
		describe("Tuple", () => {
			it("simple element", () => {
				...
				expect(schema.makeUnsafe).type.toBe<
					MakeUnsafe<readonly [string], readonly [string]>
				>();
			});
		...
		describe("Class", () => {
			it("nested defaulted fields", () => {
				...
				expect(A.makeUnsafe).type.toBe<
					MakeUnsafe<{ readonly a?: { readonly b?: number } }, A>
				>();
				const schema = Schema.Struct({
					a: A,
				});
	...
	describe("flip", () => {
		...
		it("Struct & withConstructorDefault", () => {
			const schema = Schema.Struct({
				a: Schema.String.pipe(
					Schema.withConstructorDefault(() => Option.some("c")),
				),
			});
			expect(schema.makeUnsafe).type.toBe<
				(
	...
	describe("Class", () => {
		...
		it("branded (unique symbol)", () => {
			...
			when(fABranded).isCalledWith(
				expect(BBranded.makeUnsafe).type.not.toBeCallableWith({ a: "a" }),
			);

			const fBBranded = (a: BBranded) => a;

			fBBranded(BBranded.makeUnsafe({ a: "a" }));
			when(fBBranded).isCalledWith(
				expect(ABranded.makeUnsafe).type.not.toBeCallableWith({ a: "a" }),
			);
		});
		...
		it("extend & branded (unique symbol)", () => {
			...
			class E1 extends Common.extend<E1, {}, { readonly brand: unique symbol }>(
				"E1",
			)({
				b: Schema.String,
			}) {}
			class E2 extends Common.extend<E2, {}, { readonly brand: unique symbol }>(
				"E2",
			)({
				b: Schema.String,
			}) {}
			...
			when(f1).isCalledWith(
				expect(E2.makeUnsafe).type.not.toBeCallableWith({ a: "a", b: "b" }),
			);

			const f2 = (e2: E2) => e2;

			f2(E2.makeUnsafe({ a: "a", b: "b" }));
			when(f2).isCalledWith(
				expect(E1.makeUnsafe).type.not.toBeCallableWith({ a: "a", b: "b" }),
			);
		});
	});
	...
	describe("withConstructorDefault", () => {
		it("should be possible to access the original schema", () => {
			const schema = Schema.Struct({
				a: Schema.String.pipe(
					Schema.withConstructorDefault(() => Option.some("a")),
				),
			});
	...
	it("asStandardSchemaV1 should not be callable with a schema with DecodingServices", () => {
		class MagicNumber extends ServiceMap.Service<MagicNumber, number>()(
			"MagicNumber",
		) {}
	...
	describe("fromBrand", () => {
		it("should not be callable with a schema with wrong type", () => {
			...
			when(Schema.String.pipe).isCalledWith(
				expect(Schema.fromBrand).type.not.toBeCallableWith("Int", Int),
			);
		});
