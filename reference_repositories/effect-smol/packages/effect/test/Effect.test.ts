...
describe("Effect", () => {
	...
	describe("transaction (isolated transactions)", () => {
		describe("basic isolation", () => {
			...
			it.effect(
				"should isolate failures between parent and child transactions",
				() =>
					Effect.gen(function* () {
						const ref1 = TxRef.makeUnsafe(0);
						const ref2 = TxRef.makeUnsafe(100);
						...
						const parentError = yield* Effect.transaction(
							Effect.gen(function* () {
			...
			it.effect(
				"should isolate failures from child to parent transactions",
				() =>
					Effect.gen(function* () {
						const ref1 = TxRef.makeUnsafe(0);
						const ref2 = TxRef.makeUnsafe(100);
						...
						yield* Effect.transaction(
							Effect.gen(function* () {
								...
								const childResult = yield* Effect.transaction(
									Effect.gen(function* () {
		...
		describe("transaction nesting", () => {
			it.effect(
				"should handle multiple levels of nested isolated transactions",
				() =>
					Effect.gen(function* () {
						const ref1 = TxRef.makeUnsafe(0);
		...
		describe("transactionWith function", () => {
			...
			it.effect(
				"should maintain isolation when nested in transaction blocks",
				() =>
					Effect.gen(function* () {
						const ref1 = TxRef.makeUnsafe(0);
						const ref2 = TxRef.makeUnsafe(0);
						...
						yield* Effect.transaction(
							Effect.gen(function* () {
								...
								const isolatedResult = yield* Effect.transactionWith(
									(txState) =>
										Effect.gen(function* () {
		...
		describe("transaction isolation behavior", () => {
			it.effect("should rollback entire transaction on failure", () =>
				Effect.gen(function* () {
					...
					const txError = yield* Effect.transaction(
						Effect.gen(function* () {
				...
				Effect.gen(function* () {
					...
					yield* Effect.transaction(
						Effect.gen(function* () {
							...
							const childError = yield* Effect.transaction(
								Effect.gen(function* () {
									yield* TxRef.set(ref, 20);
