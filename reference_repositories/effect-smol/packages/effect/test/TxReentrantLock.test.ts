			...
			Effect.gen(function* () {
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						assert.strictEqual(
							yield* Effect.transaction(TxReentrantLock.readLocked(lock)),
							true,
						);
					}),
				);
			...
			Effect.gen(function* () {
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						assert.strictEqual(
							yield* Effect.transaction(TxReentrantLock.writeLocked(lock)),
							true,
						);
					}),
				);
	...
	describe("withLock operations", () => {
		...
		it.effect("withLock is alias for withWriteLock", () =>
			Effect.gen(function* () {
				...
				assert.strictEqual(
					yield* Effect.transaction(TxReentrantLock.locked(lock)),
					false,
				);
			}),
		);
