...
describe("Zsh completions", () => {
	...
	it.effect("generates exclusion groups for flag aliases", () =>
		Effect.gen(function* () {
			...
			assert.include(
				script,
				"'(--loud -l --no-loud)--loud[Shout the greeting]'",
			);
...
describe("Fish completions", () => {
	...
	it.effect(
		"root-level subcommands use __fish_use_subcommand without child guard",
		() =>
			Effect.gen(function* () {
				const desc = CommandDescriptor.fromCommand(withSubcommands);
				const script = Fish.generate("server", desc);
	...
	it.effect(
		"suppresses default file completion for commands without path arguments",
		() =>
			Effect.gen(function* () {
				const desc = CommandDescriptor.fromCommand(withSubcommands);
				const script = Fish.generate("server", desc);
				...
				assert.include(
					script,
					"complete -c server -n '__fish_use_subcommand' -f",
				);
	...
	it.effect(
		"bare-TAB -a entries use double quotes around glob pattern to avoid nested single-quote errors",
		() =>
			Effect.gen(function* () {
				const desc = CommandDescriptor.fromCommand(simpleCmd);
				const script = Fish.generate("greet", desc);
	...
	it.effect(
		"does not suppress file completion for commands with path arguments",
		() =>
			Effect.gen(function* () {
				const desc = CommandDescriptor.fromCommand(withPaths);
				const script = Fish.generate("process", desc);
