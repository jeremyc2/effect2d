...
describe("Command", () => {
	describe("annotations", () => {
		it.effect("should expose annotations in help docs", () =>
			Effect.gen(function* () {
				const Team = ServiceMap.Service<never, string>(
					"effect/test/unstable/cli/Team",
				);
				const Priority = ServiceMap.Service<never, number>(
		...
		it.effect("should keep annotations when adding subcommands", () =>
			Effect.gen(function* () {
				const Scope = ServiceMap.Service<never, string>(
		...
		it.effect(
			...
			() =>
				Effect.gen(function* () {
					...
					const RegionFromProvide = ServiceMap.Service<
						never,
		...
		it.effect(
			...
			() =>
				Effect.gen(function* () {
					...
					const DbUrl = ServiceMap.Service<never, string>(
