	...
	describe("constructors", () => {
		it.effect("bounded creates pubsub with specified capacity", () =>
			Effect.gen(function* () {
				...
				assert.strictEqual(
					yield* Effect.transaction(TxPubSub.isShutdown(hub)),
					false,
				);
			}),
		);
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const fiber = yield* Effect.forkChild(
							Effect.transaction(TxPubSub.publish(hub, 3)),
						);
				...
				Effect.gen(function* () {
					...
					yield* Effect.scoped(
						Effect.gen(function* () {
							...
							yield* Effect.transaction(TxPubSub.publish(hub, 1));
							yield* Effect.transaction(TxPubSub.publish(hub, 2));
							const accepted = yield* Effect.transaction(
								TxPubSub.publish(hub, 3),
							); // should be dropped
	...
	describe("getters", () => {
			...
			Effect.gen(function* () {
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						yield* Effect.transaction(TxPubSub.publish(hub, 2));
						assert.strictEqual(
							yield* Effect.transaction(TxPubSub.size(hub)),
							2,
						);
		...
		it.effect("isEmpty checks all subscriber queues are empty", () =>
			Effect.gen(function* () {
				const hub = yield* Effect.transaction(TxPubSub.unbounded<number>());
				assert.strictEqual(
					yield* Effect.transaction(TxPubSub.isEmpty(hub)),
					true,
				);
		...
		it.effect("isFull checks if any subscriber queue is at capacity", () =>
			Effect.gen(function* () {
				const hub = yield* Effect.transaction(TxPubSub.bounded<number>(2));
				assert.strictEqual(
					yield* Effect.transaction(TxPubSub.isFull(hub)),
					false,
				);
	...
	describe("shutdown", () => {
		...
		it.effect("awaitShutdown blocks until shutdown", () =>
			Effect.gen(function* () {
				...
				const fiber = yield* Effect.forkChild(
					Effect.transaction(TxPubSub.awaitShutdown(hub)),
				);
				yield* Effect.transaction(TxPubSub.shutdown(hub));
				yield* Fiber.join(fiber);
			}),
				...
				yield* Effect.scoped(
					Effect.gen(function* () {
						...
						const f1 = yield* Effect.forkChild(
							Effect.transaction(TxPubSub.publishAll(hub, [1, 2, 3])),
						);
						const f2 = yield* Effect.forkChild(
							Effect.transaction(TxPubSub.publishAll(hub, [4, 5, 6])),
						);
