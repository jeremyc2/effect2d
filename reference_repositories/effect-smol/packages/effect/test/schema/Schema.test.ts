...
Expected a value with a size of at most 2, got Map([["a",1],["b",NaN],["c",3]])`,
  ...
  at ["a"]`,
			...
			it("Struct & Effect async & service", async () => {
				class Service extends ServiceMap.Service<
					Service,
  ...
  at ["_tag"]`,
	...
	describe("TaggedErrorClass", () => {
		...
		it("name matches identifier", () => {
			class E extends Schema.TaggedErrorClass<E>("effect/TaggedErrorName")(
				"TaggedErrorName",
				{
					id: Schema.Number,
				},
  ...
  at ["b"]`,
	...
	describe("asserts", () => {
		it("FiniteFromString", () => {
			const schema = Schema.FiniteFromString;
			const asserts: Schema.Codec.ToAsserts<typeof schema> =
...
error message 2`,
			...
			it("abort: true", async () => {
				const schema = Schema.String.check(
					Schema.makeFilter(
						() => ({ path: ["a"], message: "error message 1" }),
						{ title: "error title 1" },
						true,
					),
					Schema.makeFilter(() => false, {
  ...
  at ["a"]["b"]`,
	...
	describe("withDecodingDefault", () => {
		...
		it("should omit the value if the encoding strategy is set to omit", async () => {
			const schema = Schema.Struct({
				a: Schema.FiniteFromString.pipe(
					Schema.withDecodingDefault(() => "1", { encodingStrategy: "omit" }),
				),
			});
			const asserts = new TestSchema.Asserts(schema);
