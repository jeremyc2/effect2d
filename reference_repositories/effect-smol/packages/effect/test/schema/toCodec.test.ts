		...
		describe("schemas without encoding", () => {
			...
			it("URL", async () => {
				...
				await decoding.fail(
					"not a url",
					isDeno
						? `TypeError: Invalid URL: 'not a url'`
			...
			it("RegExp", async () => {
				...
				const encoding = asserts.encoding();
				await encoding.succeed(/a/, { source: "a", flags: "" });
				await encoding.succeed(/a/i, { source: "a", flags: "i" });

				const decoding = asserts.decoding();
				await decoding.succeed({ source: "a", flags: "" }, /a/);
				await decoding.succeed({ source: "a", flags: "i" }, /a/i);
				await decoding.fail(
					{ source: "(", flags: "" },
