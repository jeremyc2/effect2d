...
describe("Optic generation", () => {
	...
	describe("toIso", () => {
		describe("Class", () => {
			it("Class", () => {
				...
				deepStrictEqual(
					modify(
						B.makeUnsafe({
							a: A.makeUnsafe({ value: Value.makeUnsafe({ a: new Date(0) }) }),
						}),
					),
					B.makeUnsafe({
						a: A.makeUnsafe({ value: Value.makeUnsafe({ a: new Date(1) }) }),
					}),
				);
			});
		});
		...
		it("Tuple", () => {
			...
			deepStrictEqual(modify([Value.makeUnsafe({ a: new Date(0) })]), [
				Value.makeUnsafe({ a: new Date(1) }),
			]);
		});
		...
		it("suspend", () => {
			...
			deepStrictEqual(
				modify({
					a: Value.makeUnsafe({ a: new Date(0) }),
					as: [{ a: Value.makeUnsafe({ a: new Date(1) }), as: [] }],
				}),
				{
					a: Value.makeUnsafe({ a: new Date(1) }),
		...
		it("HashMap", () => {
			...
			deepStrictEqual(
				HashMap.toEntries(
					modify(HashMap.make(["a", Value.makeUnsafe({ a: new Date(0) })])),
				),
				HashMap.toEntries(
					HashMap.make(["a", Value.makeUnsafe({ a: new Date(1) })]),
				),
			);
		});
