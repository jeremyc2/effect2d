	...
	describe("constructors", () => {
		describe("make", () => {
			...
			it.effect("lookup function context is preserved", () =>
				Effect.gen(function* () {
					class TestService extends ServiceMap.Service<
						TestService,
						{ value: number }
					>()("TestService") {}
					...
					const program = Effect.gen(function* () {
						const cache = yield* ScopedCache.make({
							capacity: 10,
							lookup: (_key: string) =>
								Effect.map(TestService.asEffect(), (service) => service.value),
						});
			...
			it.effect(
				"cache is created within a scope and cleaned up when scope closes",
				() =>
					Effect.gen(function* () {
	...
	describe("service context", () => {
		describe("Service Dependency Injection", () => {
			it.effect("services are available in lookup functions", () =>
				Effect.gen(function* () {
					class ConfigService extends ServiceMap.Service<
						ConfigService,
						{ multiplier: number }
					>()("ConfigService") {}
			...
			it.effect(
				...
				() =>
					Effect.gen(function* () {
						class CounterService extends ServiceMap.Service<
							CounterService,
							{ value: number }
						>()("CounterService") {}
