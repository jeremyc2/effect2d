...
export const take = <In>(n: number): Sink<Array<In>, In, In> =>
	fromTransform((upstream) => {
		...
		let leftover: NonEmptyReadonlyArray<In> | undefined;
...
export const takeWhileEffect: <In, E, R>(
	predicate: (input: In) => Effect.Effect<boolean, E, R>,
) => Sink<Array<In>, In, In, E, R> = <In, E, R>(
	predicate: (input: In) => Effect.Effect<boolean, E, R>,
): Sink<Array<In>, In, In, E, R> =>
	fromTransform((upstream) => {
		const out = Arr.empty<In>();
		let leftover: Arr.NonEmptyReadonlyArray<In> | undefined;
		return upstream.pipe(
			Effect.flatMap((arr) => {
				let i = 0;
				return Effect.whileLoop({
...
export const provideServices: {
	...
	<A, In, L, E, R, Provided>(
		...
		fromTransform((upstream, scope) =>
			self.transform(upstream, scope).pipe(Effect.provideServices(services)),
		),
);
