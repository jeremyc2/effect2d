 ...
 * @category Models
 */
export interface Key<in out Identifier, in out Shape>
	extends Pipeable,
		Inspectable {
...
export interface Service<in out Identifier, in out Shape>
	extends Key<Identifier, Shape>,
		Yieldable<Service<Identifier, Shape>, Shape, never, Identifier> {
...
const ServiceProto: any = {
	...
	asEffect(this: any) {
		const fn = (this.asEffect = constant(
			withFiber((fiber) => exitSucceed(get(fiber.services, this))),
		));
		return fn();
	},
...
const Proto: Omit<ServiceMap<never>, "mapUnsafe" | "mutable"> = {
	...
	[Equal.symbol]<A>(this: ServiceMap<A>, that: unknown): boolean {
		if (!isServiceMap(that) || this.mapUnsafe.size !== that.mapUnsafe.size)
			return false;
 ...
 * @category Guards
 */
export const isServiceMap = (u: unknown): u is ServiceMap<never> =>
...
export const getOrElse: {
	<S, I, B>(
		key: Key<I, S>,
		orElse: LazyArg<B>,
	): <Services>(self: ServiceMap<Services>) => S | B;
	<Services, S, I, B>(
		self: ServiceMap<Services>,
		key: Key<I, S>,
		orElse: LazyArg<B>,
...
export const getOrUndefined: {
	<S, I>(
		key: Key<I, S>,
	): <Services>(self: ServiceMap<Services>) => S | undefined;
...
export const getOption: {
	<S, I>(
		service: Key<I, S>,
	): <Services>(self: ServiceMap<Services>) => Option.Option<S>;
	<Services, S, I>(
		self: ServiceMap<Services>,
		service: Key<I, S>,
	): Option.Option<S>;
...
export const merge: {
	<R1>(
		that: ServiceMap<R1>,
	): <Services>(self: ServiceMap<Services>) => ServiceMap<R1 | Services>;
	<Services, R1>(
		self: ServiceMap<Services>,
		that: ServiceMap<R1>,
	): ServiceMap<Services | R1>;
} = dual(
	2,
	<Services, R1>(
		self: ServiceMap<Services>,
...
export const pick =
	...
	<Services>(
		self: ServiceMap<Services>,
	): ServiceMap<Services & Service.Identifier<S[number]>> =>
		withMapUnsafe(self, (map) => {
...
export const omit =
	...
	<Services>(
		self: ServiceMap<Services>,
	): ServiceMap<Exclude<Services, Service.Identifier<S[number]>>> =>
		withMapUnsafe(self, (map) => {
