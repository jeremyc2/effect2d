...
export const fiberInterruptAs: {
	...
	<A, E>(
		...
		withFiber((parent) => {
			...
			ann =
				ann && annotations
					? ServiceMap.merge(ann, annotations)
					: (ann ?? annotations);
...
) => Effect.Effect<A, Cause.NoSuchElementError> = fromYieldable;

/** @internal */
export const fromResult: <A, E>(
	result: Result.Result<A, E>,
) => Effect.Effect<A, E> = fromYieldable;
...
export const promise = <A>(
	...
	callbackOptions<A>((resume, signal) => {
...
export const tryPromise = <A, E = Cause.UnknownError>(
	...
	const catcher =
		typeof options === "function"
			...
			: options.catch;
	return callbackOptions<A, E>((resume, signal) => {
		try {
...
>(
	...
	[Eff] extends [never]
		? never
		: [Eff] extends [Effect.Yieldable<infer _A, infer E, infer _R>]
			? E
			: never,
	[Eff] extends [never]
		? never
		: [Eff] extends [Effect.Yieldable<infer _A, infer _E, infer R>]
			? R
			: never
> =>
	suspend(() =>
...
) => Effect.Effect<S, never, I> = fromYieldable as any;

/** @internal */
export const serviceOption = <I, S>(
	service: ServiceMap.Key<I, S>,
): Effect.Effect<Option.Option<S>> =>
	withFiber((fiber) => succeed(ServiceMap.getOption(fiber.services, service)));
...
export const updateServices: {
	<R2, R>(
		...
		) => ServiceMap.ServiceMap<NoInfer<R>>,
...
/** @internal */
export const services = <R = never>(): Effect.Effect<
	ServiceMap.ServiceMap<R>
> => getServiceMap as any;
...
export const servicesWith = <R, A, E, R2>(
	...
	withFiber((fiber) => f(fiber.services as ServiceMap.ServiceMap<R>));
...
export const timeoutOrElse: {
	...
	<A, E, R, A2, E2, R2>(
		...
		raceFirst(self, flatMap(sleep(options.duration), options.onTimeout)),
);
...
/** @internal */
export const scopeTag: ServiceMap.Service<Scope.Scope, Scope.Scope> =
	ServiceMap.Service<Scope.Scope>("effect/Scope");
			...
			flatMap(scope, (scope) =>
				tap(options?.interruptible ? restore(acquire) : acquire, (a) =>
					scopeAddFinalizerExit(scope, (exit) =>
						provideServices(release(a, exit), services),
					),
				),
			),
		),
...
export const addFinalizer = <R>(
	...
	flatMap(scope, (scope) =>
		servicesWith((services: ServiceMap.ServiceMap<R>) =>
			scopeAddFinalizerExit(scope, (exit) =>
				provideServices(finalizer(exit), services),
			),
		),
	);
...
>(
	...
	: <A, E, R>(
			self: Effect.Effect<A, E, R>,
		) => Effect.Effect<Fiber.Fiber<A, E>, never, R> = dual(
	(args) => isEffect(args[0]),
	<A, E, R>(
		self: Effect.Effect<A, E, R>,
		options?: {
...
>(
	...
	: <A, E, R>(
			self: Effect.Effect<A, E, R>,
		) => Effect.Effect<Fiber.Fiber<A, E>, never, R | Scope.Scope> = dual(
	(args) => isEffect(args[0]),
	<A, E, R>(
		self: Effect.Effect<A, E, R>,
		options?: {
...
export const runForkWith =
	<R>(services: ServiceMap.ServiceMap<R>) =>
	<A, E>(
		effect: Effect.Effect<A, E, R>,
		...
		const fiber = new FiberImpl<A, E>(
			options?.scheduler
				? ServiceMap.add(services, Scheduler.Scheduler, options.scheduler)
				: services,
...
export const runSyncExit: <A, E>(
	effect: Effect.Effect<A, E>,
) => Exit.Exit<A, E> = runSyncExitWith(ServiceMap.empty());
...
class Latch implements _Latch.Latch {
	...
	release = withFiber<boolean>((fiber) =>
		this.open ? succeedFalse : this.scheduleUnsafe(fiber),
	);
	openUnsafe() {
...
export const makeSpanUnsafe = <XA, XE>(
	...
	const disablePropagation =
		...
		(options?.annotations &&
			ServiceMap.get(options.annotations, Tracer.DisablePropagation));
	const parent =
...
/** @internal */
export const LogToStderr = ServiceMap.Reference<boolean>(
...
export const annotateLogsScoped: {
	...
	return uninterruptible(
		withFiber((fiber) => {
			...
			fiber.setServices(
				ServiceMap.add(fiber.services, CurrentLogAnnotations, next),
			);
			return scopeAddFinalizerExit(
				ServiceMap.getUnsafe(fiber.services, scopeTag),
				(_) => {
