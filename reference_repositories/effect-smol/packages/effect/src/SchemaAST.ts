			...
			for (; i < elementLen; i++) {
				...
				if (exit._tag === "Failure") {
					...
					const issue = new Issue.Pointer(
						[i],
						new Issue.MissingKey(e.ast.context?.annotations),
					);
					if (errorsAllOption) {
						if (issues) issues.push(issue);
						else issues = [issue];
			...
			if (ast.rest.length > 0) {
				...
				for (; i < len - tail.length; i++) {
					...
					if (exit._tag === "Failure") {
						...
						const issue = new Issue.Pointer(
							[i],
							new Issue.MissingKey(keyAnnotations),
						);
						if (errorsAllOption) {
							if (issues) issues.push(issue);
							else issues = [issue];
			...
			for (let i = 0; i < candidates.length; i++) {
				...
				if (exit._tag === "Failure") {
					...
					if (tracking.out && oneOf) {
						...
						return yield* Effect.fail(
							new Issue.OneOf(ast, input, tracking.successes),
						);
					}
...
export function withConstructorDefault<A extends AST>(
	...
	) => Option.Option<unknown> | Effect.Effect<Option.Option<unknown>>,
): A {
