...
export const offerAll: {
	<A, E>(
		...
		self: TxEnqueue<A, E>,
	) => Effect.Effect<Array<A>, never, Effect.Transaction>;
	<A, E>(
		self: TxEnqueue<A, E>,
		values: Iterable<A>,
	): Effect.Effect<Array<A>, never, Effect.Transaction>;
...
export const poll = <A, E>(
	self: TxDequeue<A, E>,
): Effect.Effect<Option.Option<A>, never, Effect.Transaction> =>
	Effect.gen(function* () {
		const state = yield* TxRef.get(self.stateRef);
		if (state._tag === "Done") {
			return Option.none();
...
export const takeAll = <A, E>(
	self: TxDequeue<A, E>,
): Effect.Effect<Arr.NonEmptyArray<A>, E, Effect.Transaction> =>
	Effect.gen(function* () {
		const state = yield* TxRef.get(self.stateRef);
...
export const takeN: {
	(
		...
		self: TxDequeue<A, E>,
	) => Effect.Effect<Array<A>, E, Effect.Transaction>;
	<A, E>(
		self: TxDequeue<A, E>,
		n: number,
	): Effect.Effect<Array<A>, E, Effect.Transaction>;
...
export const size = (
	self: TxQueueState,
): Effect.Effect<number, never, Effect.Transaction> => TxChunk.size(self.items);
...
export const clear = <A, E>(
	self: TxEnqueue<A, E>,
): Effect.Effect<Array<A>, ExcludeDone<E>, Effect.Transaction> =>
	Effect.gen(function* () {
		const state = yield* TxRef.get(self.stateRef);
		if (state._tag === "Done") {
...
export const isShutdown = (
	self: TxQueueState,
): Effect.Effect<boolean, never, Effect.Transaction> => isDone(self);
