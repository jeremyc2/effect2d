 ...
 * @category server
 */
export class McpServer extends ServiceMap.Service<
	McpServer,
	{
					...
					switch (request._tag) {
						case "Request": {
							if (isHttp) {
								...
								const httpRequest = ServiceMap.getUnsafe(
									fiber.services,
			...
			return ClientRpcs.of({
				...
				initialize(params, { clientId }) {
					...
					return Effect.withFiber((fiber) => {
						const httpRequest = ServiceMap.getOrUndefined(
							fiber.services,
							HttpServerRequest.HttpServerRequest,
						);
						if (httpRequest) {
				...
				"prompts/list": (_, { clientId, headers }) =>
					Effect.sync(() => {
						...
						return new ListPromptsResult({
							prompts: filterByClient(client, server.prompts, "prompt"),
						});
					}),
				"resources/list": (_, { clientId, headers }) =>
					Effect.sync(() => {
						...
						return new ListResourcesResult({
							resources: filterByClient(client, server.resources, "resource"),
						});
					}),
				"resources/read": ({ uri }) =>
					server
...
>(
	...
	for (let i = 0; i < items.length; i++) {
		const item = items[i];
		const enabledWhen = ServiceMap.getOrUndefined(
			item.annotations,
			EnabledWhen,
		);
