 ...
 * @category tags
 */
export const Socket: ServiceMap.Service<Socket, Socket> =
	ServiceMap.Service<Socket>("effect/socket/Socket");
 ...
 * @category tags
 */
export class WebSocket extends ServiceMap.Service<
	WebSocket,
	...
	Effect.withFiber((fiber) => {
		...
		const runRaw = <_, E, R>(
			...
			Effect.scopedWith(
				...
				Effect.updateServices((input: ServiceMap.ServiceMap<R>) =>
					ServiceMap.merge(acquireContext, input),
				),
				Effect.ensuring(
					Effect.sync(() => {
						latch.closeUnsafe();
 ...
 * @category fiber refs
 */
export const SendQueueCapacity = ServiceMap.Reference<number>(
	...
	Effect.withFiber((fiber) => {
		...
		const runRaw = <_, E, R>(
			...
			Effect.scopedWith(
				...
				(_) => _,
				Effect.updateServices((input: ServiceMap.ServiceMap<R>) =>
					ServiceMap.merge(acquireServices, input),
				),
				Effect.ensuring(
					Effect.sync(() => {
						latch.closeUnsafe();
