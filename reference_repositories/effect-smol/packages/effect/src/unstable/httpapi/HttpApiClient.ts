 ...
 * @category models
 */
export type ForApi<Api extends HttpApi.Any, E = BadRequest, R = never> =
	...
	export type Method<Endpoint, E, R> = [Endpoint] extends [
		...
		? <WithResponse extends boolean = false>(
				...
				WithResponse extends true
					? [_Success["Type"], HttpClientResponse.HttpClientResponse]
					: _Success["Type"],
				| _Error["Type"]
...
type EndpointWithId<
	...
	Id extends string,
> = Id extends `${infer Method extends HttpMethod.HttpMethod} ${infer Path extends string}`
	? Extract<Endpoints, { readonly method: Method; readonly path: Path }>
	: never;

type UrlBuilderRequest<Endpoint extends HttpApiEndpoint.Any> = ([
	HttpApiEndpoint.Params<Endpoint>["Encoded"],
] extends [never]
	? {}
...
export type UrlBuilder<Api extends HttpApi.Any> = <
	...
	const Id extends EndpointId<
		HttpApiGroup.EndpointsWithName<ApiGroups<Api>, GroupName>
	>,
>(
	group: GroupName,
	endpoint: Id,
...
export const urlBuilder = <Api extends HttpApi.Any>(options?: {
	...
	return ((
		...
		const withParams =
			request?.params === undefined ? path : compilePath(path)(request.params);
		const query =
			request?.query === undefined
				? ""
				: UrlParams.toString(UrlParams.fromInput(request.query));
...
function getEncodePayloadSchemaFromBody(schema: Schema.Top): Schema.Top {
