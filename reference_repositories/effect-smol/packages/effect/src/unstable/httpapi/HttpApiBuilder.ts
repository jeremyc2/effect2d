	...
	HttpRouter.use(
		Effect.fnUntraced(function* (router) {
			...
			for (const group of Object.values(api.groups)) {
				const groupRoutes = services.mapUnsafe.get(group.key) as Array<
...
> =>
	Effect.servicesWith((services: ServiceMap.ServiceMap<any>) => {
		...
		return Effect.succeed(
			handlerToHttpEffect(group, endpoint, services, handler as any, false),
		);
	});
...
function handlerToHttpEffect(
	...
	return applyMiddleware(
		...
		Effect.gen(function* () {
			...
			const routeContext = ServiceMap.getUnsafe(
				services,
				HttpRouter.RouteContext,
			);
...
const applyMiddleware = <A extends Effect.Effect<any, any, any>>(
	...
	for (const key_ of endpoint.middlewares) {
		...
		const service = ServiceMap.getUnsafe(
			services,
			key as any,
