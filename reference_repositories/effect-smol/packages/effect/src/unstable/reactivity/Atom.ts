...
const makeRead: {
	...
	<A>(
		create: (get: Context) => A,
	): (get: Context, services?: ServiceMap.ServiceMap<any>) => A;
	<A>(initialValue: A): Writable<A>;
} = <A, E>(
	arg:
	...
	if (
		...
		return (get: Context, providedServices?: ServiceMap.ServiceMap<any>) => {
		...
		return (get: Context, providedServices?: ServiceMap.ServiceMap<any>) =>
			effect(get, arg as any, options, providedServices);
	} else if (Stream.isStream(arg)) {
		return (get: Context, providedServices?: ServiceMap.ServiceMap<any>) =>
			stream(get, arg as any, options, providedServices);
	}
...
const state = <A>(initialValue: A): Writable<A> =>
	writable((_get) => initialValue, constSetSelf);
...
export interface AtomRuntime<R, ER = never>
	extends Atom<AsyncResult.AsyncResult<ServiceMap.ServiceMap<R>, ER>> {
...
export const subscriptionRef: {
	<A>(
		ref:
			| SubscriptionRef.SubscriptionRef<A>
			| ((get: Context) => SubscriptionRef.SubscriptionRef<A>),
	): Writable<A>;
	<A, E>(
		effect:
			| Effect.Effect<
...
export const pull = <A, E>(
	...
	const pullAtom = readable(
		makeRead((get) => makeStreamPullEffect(get, pullSignal, create, options)),
	);
	return makeStreamPull(pullSignal, pullAtom);
};
...
> =>
	Effect.flatMap(
		...
		(pullChunk) => {
			...
			const services = fiber.services as ServiceMap.ServiceMap<
