	...
	>(
		...
		? Atom.AtomResultFn<
				...
				WithResponse extends true
					? [_Success["Type"], HttpClientResponse]
					: _Success["Type"],
				_Error["Type"]
			>
		: never;
...
export const Service =
	...
	>(
		...
		const self: Mutable<AtomHttpApiClient<Self, Id, Groups>> =
			ServiceMap.Service<Self, HttpApiClient.Client<Groups, never, never>>()(
				id,
			) as any;
		...
		const mutationFamily = Atom.family(
			({ endpoint, group, withResponse }: MutationKey) => {
...
const makeSerializableKey = (key: QueryKey): string =>
	`AtomHttpApi:${key.group}:${key.endpoint}:${Hash.hash(key)}`;
