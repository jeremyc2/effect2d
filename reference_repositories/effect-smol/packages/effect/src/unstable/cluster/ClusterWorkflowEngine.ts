...
export const make = Effect.gen(function* () {
	...
	const requestIdFor = Effect.fnUntraced(function* (options: {
		...
		const shardGroup = ServiceMap.get(
	...
	const engine = WorkflowEngine.makeUnsafe({
						...
						Effect.gen(function* () {
							...
							return {
								run: (request: Entity.Request<any>) => {
									...
									return execute(
										workflow.payloadSchema.makeUnsafe(payload) as object,
										executionId,
									).pipe(
										Effect.onExit((exit) => {
		...
		activityExecute: Effect.fnUntraced(function* (activity, attempt) {
			const services =
				yield* Effect.services<WorkflowEngine.WorkflowInstance>();
			const instance = ServiceMap.get(
				services,
				WorkflowEngine.WorkflowInstance,
			);
