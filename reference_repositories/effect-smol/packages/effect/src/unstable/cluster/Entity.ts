...
const Proto = {
	...
	annotateMerge<S>(
		this: Entity<string, any>,
		annotations: ServiceMap.ServiceMap<S>,
	) {
	...
	getShardId(this: Entity<string, any>, entityId: EntityId) {
		return Effect.map(shardingTag.asEffect(), (sharding) =>
	...
	get client() {
		return shardingTag
			.asEffect()
			.pipe(Effect.flatMap((sharding) => sharding.makeClient(this as any)));
	},
	toLayer<
	...
	export type Success<R extends Rpc.Any> =
		Rpc.Success<R> extends Stream.Stream<infer _A, infer _E, infer _R>
			?
					...
					| Queue.Dequeue<_A, _E | Rpc.Error<R>>
			: Rpc.Success<R>;
}
...
const shardingTag = ServiceMap.Service<Sharding, Sharding["Service"]>(
 ...
 * @category Keep alive
 */
export class KeepAliveLatch extends ServiceMap.Service<
	KeepAliveLatch,
	Latch.Latch
