 ...
 * @category models
 */
export class Sharding extends ServiceMap.Service<
	Sharding,
	{
		/**
...
const make = Effect.gen(function* () {
	...
	const services = ServiceMap.omit(Scope.Scope)(
		yield* Effect.services<ShardingConfig>(),
	);
	const runFork = flow(
		...
		Effect.fnUntraced(function* (entity: Entity<string, any>) {
			const client = yield* RpcClient.makeNoSerialization(entity.protocol, {
				...
				onFromClient(
					...
					MailboxFull | AlreadyProcessingMessage | PersistenceError
				> {
					const address = ServiceMap.getUnsafe(
						options.context,
						ClientAddressTag,
					);
					switch (options.message._tag) {
						...
						case "Interrupt": {
							...
							if (
								ClusterSchema.isUninterruptibleForClient(entry.rpc.annotations)
							) {
								return Effect.void;
							}
							...
							if (
								...
								ServiceMap.get(entry.rpc.annotations, Persisted)
							) {
								return Effect.void;
							}
			...
			return (entityId: string) => {
				...
				const address = ClientAddressTag.serviceMap(
					makeEntityAddress({
				...
				const clientFn = (
					...
					const context = options?.context
						? ServiceMap.merge(options.context, address)
						: address;
...
const ClientAddressTag = ServiceMap.Service<EntityAddress>(
