...
const RpcGroupProto = {
	...
	accessHandler(this: RpcGroup<any>, service: string) {
		return Effect.servicesWith((parentServices: ServiceMap.ServiceMap<any>) => {
			...
			const { handler, services } = parentServices.mapUnsafe.get(
				rpc.key,
			) as Rpc.Handler<any>;
	...
	annotateRpcs(
		this: RpcGroup<any>,
		service: ServiceMap.Key<any, any>,
		value: any,
	) {
	...
	annotateRpcsMerge(
		this: RpcGroup<any>,
		serviceMap: ServiceMap.ServiceMap<any>,
	) {
		const requests = new Map<string, any>();
		for (const [tag, rpc] of this.requests) {
			requests.set(
				tag,
				rpc.annotateMerge(ServiceMap.merge(serviceMap, rpc.annotations)),
			);
		}
		return makeProto({
			requests,
