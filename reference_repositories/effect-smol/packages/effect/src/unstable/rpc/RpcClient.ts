	...
	const sendInterrupt = (
		...
		Effect.callback<void>((resume) => {
			...
			const fiber = options
				...
				.pipe(Effect.timeout(1000), Effect.runForkWith(parentFiber.services));
			fiber.addObserver(() => {
				resume(Effect.void);
			});
		});
 ...
 * @category headers
 */
export const CurrentHeaders = ServiceMap.Reference<Headers.Headers>(
...
> =>
	Layer.effect(Protocol)(
		Effect.flatMap(HttpClient.HttpClient.asEffect(), (client) => {
