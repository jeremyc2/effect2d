 ...
 */
export const RouterConfig = ServiceMap.Reference<
	Partial<FindMyWay.RouterConfig>
>("effect/http/HttpRouter/RouterConfig", { defaultValue: () => ({}) });
...
> => {
	...
	return Effect.servicesWith(
		(
			...
			const request = ServiceMap.get(
				services,
				HttpServerRequest.HttpServerRequest,
			);
			const searchParams = ServiceMap.get(
				services,
				HttpServerRequest.ParsedSearchParams,
			);
				...
				Effect.isEffect(middleware)
					? Layer.effectServices(
							Effect.map(middleware, (fn) =>
								ServiceMap.makeUnsafe(new Map([[fnContextKey, fn]])),
							),
						)
					: (Layer.succeedServices(
							ServiceMap.makeUnsafe(new Map([[fnContextKey, middleware]])),
						) as any),
			);
...
>(
	...
	readonly handler: [HR] extends [never]
		? (
				...
				context?: ServiceMap.ServiceMap<never> | undefined,
			) => Promise<Response>
		: (
				request: globalThis.Request,
				context: ServiceMap.ServiceMap<HR>,
			) => Promise<Response>;
	...
	return HttpEffect.toWebHandlerLayerWith(
		...
		{
			toHandler: (s) =>
				Effect.succeed(ServiceMap.get(s, HttpRouter).asHttpEffect()),
			middleware,
