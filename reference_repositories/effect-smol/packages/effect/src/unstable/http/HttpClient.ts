...
const accessor =
	...
	(...args: Array<any>): Effect.Effect<any, any, any> =>
		Effect.flatMap(HttpClient.asEffect(), (client) =>
...
export const makeWith = <E2, R2, E, R>(
	...
	self.execute = (request: HttpClientRequest.HttpClientRequest) =>
 ...
 * @category References
 */
export const TracerPropagationEnabled = ServiceMap.Reference<boolean>(
	...
	Layer.effect(HttpClient)(
		Effect.servicesWith((services: ServiceMap.ServiceMap<never>) =>
			Effect.map(effect, (client) =>
				transformResponse(
					client,
					Effect.updateServices((input: ServiceMap.ServiceMap<never>) =>
