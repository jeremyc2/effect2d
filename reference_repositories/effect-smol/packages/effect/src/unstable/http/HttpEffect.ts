		...
		middleware === undefined
			...
			: Effect.matchCauseEffect(tracer(middleware(responded)), {
					onFailure(cause): Effect.Effect<void, EH, RH> {
						const fiber = Fiber.getCurrent()!;
						reportCauseUnsafe(fiber, cause);
						const request = ServiceMap.getUnsafe(
							fiber.services,
							HttpServerRequest,
						);
...
export const scopeTransferToStream = (
	...
	const fiber = Fiber.getCurrent()!;
	const scope = ServiceMap.getUnsafe(
		fiber.services,
		Scope.Scope,
	) as Scope.Closeable;
...
export const toWebHandlerWith =
	...
	<E>(
		...
		return (
			request: Request,
			reqServices?: ServiceMap.ServiceMap<never> | undefined,
		): Promise<globalThis.Response> =>
			new Promise((resolve) => {
				...
				const fiber = Effect.runForkWith(ServiceMap.makeUnsafe(contextMap))(
					httpApp as any,
				);
...
export const toWebHandler: <E>(
	...
	request: Request,
	services?: ServiceMap.ServiceMap<never> | undefined,
) => Promise<globalThis.Response> = toWebHandlerWith(ServiceMap.empty());
