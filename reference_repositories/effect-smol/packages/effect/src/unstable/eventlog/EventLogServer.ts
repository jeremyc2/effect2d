	...
	return Effect.fnUntraced(
		function* (socket: Socket.Socket) {
			...
			const write = Effect.fnUntraced(function* (
				response: Schema.Schema.Type<typeof ProtocolResponse>,
			) {
			...
			const handleRequest = (
				request: Schema.Schema.Type<typeof ProtocolRequest>,
			): Effect.Effect<void> => {
				switch (request._tag) {
					case "Ping": {
					...
					case "WriteEntries": {
						...
						return Effect.gen(function* () {
							...
							const encrypted = yield* storage.write(
								request.publicKey,
								entries,
							);
					...
					case "ChunkedMessage": {
						...
						return Effect.flatMap(
							Effect.orDie(decodeRequest(data)),
							handleRequest,
						);
					}
				}
			...
			yield* socket
				.run((data) =>
					Effect.flatMap(Effect.orDie(decodeRequest(data)), handleRequest),
				)
...
export class Storage extends ServiceMap.Service<
	Storage,
	{
		...
		readonly changes: (
			...
			Queue.Dequeue<EncryptedRemoteEntry, Cause.Done>,
			never,
			Scope.Scope
		>;
	}
...
> = Effect.gen(function* () {
	...
	return Storage.of({
		...
		entries: (publicKey, startSequence) =>
			Effect.sync(() => ensureJournal(publicKey).slice(startSequence)),
		changes: (publicKey, startSequence) =>
			Effect.gen(function* () {
				...
				yield* Queue.offerAll(
					queue,
					ensureJournal(publicKey).slice(startSequence),
				);
