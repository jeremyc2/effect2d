...
export const group = <Events extends Event.Any, Return>(
	...
	Exclude<Handlers.Services<Return>, Scope.Scope>
> =>
...
export const groupCompaction = <Events extends Event.Any, R>(
	...
	never,
	never,
	Identity | EventJournal | R | Event.PayloadSchema<Events>["DecodingServices"]
> =>
	Layer.effectDiscard(
		Effect.gen(function* () {
...
const make = Effect.gen(function* () {
								...
								compactors.size > 0
									? Effect.fnUntraced(function* (remoteEntries) {
											...
											while (index < remoteEntries.length) {
												...
												const compactor = compactors.get(
													remoteEntry.entry.event,
												);
												if (!compactor) {
												...
												while (index < remoteEntries.length) {
													...
													if (
														!compactor.events.has(nextRemoteEntry.entry.event)
													) {
														break;
													}
							...
							effect: Effect.fnUntraced(
								function* ({
									...
									const handler = services.mapUnsafe.get(
										Event.serviceKey(entry.event),
									) as Handlers.Item<any> | undefined;
									if (!handler) {
									...
									for (let i = 0; i < conflicts.length; i++) {
										decodedConflicts[i] = {
											...
											payload: yield* decodePayload(conflicts[i].payload).pipe(
												Effect.updateServices((input) =>
													ServiceMap.merge(handler.services, input),
												),
											) as any,
										};
									}
	...
	const eventLogWrite = (options: {
		...
		const handler = services.mapUnsafe.get(Event.serviceKey(options.event)) as
			| Handlers.Item<any>
			| undefined;
...
> = Layer.effect(EventLog, make).pipe(Layer.provide(ReactivityLayer.layer));
