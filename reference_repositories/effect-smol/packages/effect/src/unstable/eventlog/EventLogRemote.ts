...
import {
	...
	EventLogEncryption,
	layerSubtle,
} from "./EventLogEncryption.ts";

/**
 * @since 4.0.0
 * @category models
 */
export class EventLogRemote extends ServiceMap.Service<
	EventLogRemote,
	{
 ...
 * @category protocol
 */
export class Hello extends Schema.Class<Hello>(
	"effect/eventlog/EventLogRemote/Hello",
)({
 ...
 * @category protocol
 */
export class ChunkedMessage extends Schema.Class<ChunkedMessage>(
 ...
 * @category protocol
 */
export class Ack extends Schema.Class<Ack>(
	"effect/eventlog/EventLogRemote/Ack",
)({
	_tag: Schema.tag("Ack"),
	id: Schema.Number,
 ...
 * @category protocol
 */
export class RequestChanges extends Schema.Class<RequestChanges>(
	"effect/eventlog/EventLogRemote/RequestChanges",
)({
 ...
 * @category protocol
 */
export class Changes extends Schema.Class<Changes>(
	"effect/eventlog/EventLogRemote/Changes",
)({
 ...
 * @category protocol
 */
export class StopChanges extends Schema.Class<StopChanges>(
	"effect/eventlog/EventLogRemote/StopChanges",
)({
 ...
 * @category protocol
 */
export class Ping extends Schema.Class<Ping>(
	"effect/eventlog/EventLogRemote/Ping",
)({
 ...
 * @category protocol
 */
export class Pong extends Schema.Class<Pong>(
	"effect/eventlog/EventLogRemote/Pong",
)({
 ...
 * @category protocol
 */
export const encodeResponse = Schema.encodeUnknownEffect(
	ProtocolResponseMsgpack,
);
 ...
 * @category change
 */
export class RemoteAdditions extends Schema.Class<RemoteAdditions>(
	"effect/eventlog/EventLogRemote/RemoteAdditions",
)({
 ...
 * @category entry
 */
export const RemoteEntryChange = Schema.Tuple([
	RemoteId,
	Schema.Array(EntryId),
]);
...
export const fromSocket = (options?: {
	...
	never,
	Scope.Scope | EventLogEncryption | Socket.Socket
> =>
	Effect.gen(function* () {
		...
		const writeRequest = (request: typeof ProtocolRequest.Type) =>
			Effect.gen(function* () {
				...
				if (
					request._tag !== "WriteEntries" ||
					data.byteLength <= constChunkSize
				) {
		...
		const subscriptions = yield* RcMap.make({
			lookup: (publicKey: string) =>
				Effect.acquireRelease(Queue.make<RemoteEntry>(), (queue) =>
			...
			switch (res._tag) {
				...
				case "Ack": {
					return Effect.gen(function* () {
						...
						const remoteEntries = res.sequenceNumbers.map(
							(sequenceNumber, i) => {
		...
		yield* socket
			.run((data) => Effect.flatMap(decodeResponse(data), handleMessage))
			.pipe(
					...
					yield* Effect.orDie(
						writeRequest(
							new WriteEntries({
								...
								id,
								iv: encrypted.iv,
								encryptedEntries: encrypted.encryptedEntries.map(
									(encryptedEntry, i) => ({
...
export const fromWebSocket = (
	...
	EventLogRemote["Service"],
	never,
	Scope.Scope | EventLogEncryption | Socket.WebSocketConstructor
> =>
	Effect.gen(function* () {
...
export const layerWebSocket = (
	...
	never,
	never,
	Socket.WebSocketConstructor | EventLogEncryption
