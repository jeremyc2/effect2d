...
> extends ServiceMap.ServiceClass<Self, Id, LayerMap<K, I, E>> {
	...
	readonly services: (
		key: K,
	) => Effect.Effect<ServiceMap.ServiceMap<I>, E, Scope.Scope | Self>;
...
export const Service =
	...
	> => {
		...
		TagClass_.get = (key: string) =>
			Layer.unwrap(
				Effect.map(TagClass_.asEffect(), (layerMap) => layerMap.get(key)),
			);
		TagClass_.services = (key: string) =>
			Effect.flatMap(TagClass_.asEffect(), (layerMap) =>
				layerMap.services(key),
			);
		TagClass_.invalidate = (key: string) =>
			Effect.flatMap(TagClass_.asEffect(), (layerMap) =>
				layerMap.invalidate(key),
			);
