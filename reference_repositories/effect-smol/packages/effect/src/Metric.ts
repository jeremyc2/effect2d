 ...
 * @category References
 */
export const CurrentMetricAttributes =
	ServiceMap.Reference<Metric.AttributeSet>(CurrentMetricAttributesKey, {
		defaultValue: () => ({}),
	});
 ...
 * @category References
 */
export const MetricRegistry = ServiceMap.Reference<
...
abstract class Metric$<in Input, out State> implements Metric<Input, State> {
	...
	valueUnsafe(context: ServiceMap.ServiceMap<never>): State {
		return this.hook(context).get(context);
	}

	modifyUnsafe(input: Input, context: ServiceMap.ServiceMap<never>): void {
		return this.hook(context).modify(input, context);
	}

	updateUnsafe(input: Input, context: ServiceMap.ServiceMap<never>): void {
...
export const update: {
	...
	<Input, State>(self: Metric<Input, State>, input: Input) => Effect<void>
>(2, (self, input) =>
	InternalEffect.servicesWith((services) =>
...
>(
	...
	<Input, State>(
		...
		new MetricTransform(
			self,
			(context) =>
				self.valueUnsafe(addAttributesToServiceMap(context, attributes)),
			(input, context) =>
				self.updateUnsafe(
					input,
					addAttributesToServiceMap(context, attributes),
				),
			(input, context) =>
				self.modifyUnsafe(
					input,
...
export const snapshot: Effect<ReadonlyArray<Metric.Snapshot>> =
	InternalEffect.map(InternalEffect.services(), (context) =>
...
export const exponentialBoundaries = (options: {
	...
	boundariesFromIterable(
		Arr.makeBy(options.count - 1, (i) => options.start * options.factor ** i),
	);
 ...
 * @category Runtime Metrics
 */
export const FiberRuntimeMetrics = ServiceMap.Reference<
