 ...
 * @category models
 */
export class CurrentMemoMap extends ServiceMap.Service<
	CurrentMemoMap,
	MemoMap
>()("effect/Layer/CurrentMemoMap") {
	static getOrCreate: <Services>(
		self: ServiceMap.ServiceMap<Services>,
	) => MemoMap = ServiceMap.getOrElse(this, makeMemoMapUnsafe);
}
	...
	<RIn, E, ROut>(
		...
		internalEffect.provideService(
			internalEffect.map(
				...
				ServiceMap.add(CurrentMemoMap, memoMap),
			),
			CurrentMemoMap,
			memoMap,
		),
...
export const succeed: {
	...
	if (arguments.length === 1) {
		return (resource: any) =>
			succeedServices(ServiceMap.make(arguments[0], resource));
	}
 ...
 * @category constructors
 */
export const succeedServices = <A>(
	services: ServiceMap.ServiceMap<A>,
): Layer<A> => fromBuildUnsafe(constant(internalEffect.succeed(services)));
...
export const sync: {
	...
	if (arguments.length === 1) {
		return (evaluate: LazyArg<any>) =>
			syncServices(() => ServiceMap.make(arguments[0], evaluate()));
	}
 ...
 * @category constructors
 */
export const syncServices = <A>(
	evaluate: LazyArg<ServiceMap.ServiceMap<A>>,
...
const effectImpl = <I, S, E, R>(
	...
	effectServices(
		internalEffect.map(effect, (value) => ServiceMap.make(service, value)),
	);
...
> => {
	...
	return internalEffect
		...
		.pipe(
			internalEffect.map((services) =>
				ServiceMap.mergeAll(...(services as any)),
			),
		);
};
...
export const tap: {
	...
	<RIn, E, ROut, XR extends ROut, RIn2, E2, X>(
		...
		fromBuild((memoMap, scope) =>
			internalEffect.flatMap(self.build(memoMap, scope), (context) =>
				Scope.provide(
					internalEffect.as(f(context as ServiceMap.ServiceMap<XR>), context),
					scope,
				),
			),
		),
...
export const updateService: {
	...
	<A1, E1, R1, I, A>(
		...
		provide(layer, effect(service)(internalEffect.map(service.asEffect(), f))),
);
