...
export const bounded = <A = never>(
	capacity: number,
): Effect.Effect<TxPubSub<A>, never, Effect.Transaction> =>
	Effect.gen(function* () {
 ...
 * @category constructors
 */
export const unbounded = <A = never>(): Effect.Effect<
	TxPubSub<A>,
	never,
	Effect.Transaction
> =>
...
export const publish: {
	<A>(
		value: A,
	): (self: TxPubSub<A>) => Effect.Effect<boolean, never, Effect.Transaction>;
	<A>(
		self: TxPubSub<A>,
		value: A,
	): Effect.Effect<boolean, never, Effect.Transaction>;
...
export const subscribe = <A>(
	...
	Effect.acquireRelease(Effect.transaction(acquireSubscriber(self)), (queue) =>
		Effect.transaction(releaseSubscriber(self, queue)),
	);
