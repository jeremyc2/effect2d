...
export const make = <A>(
	value: A,
): Effect.Effect<TxSubscriptionRef<A>, never, Effect.Transaction> =>
	Effect.gen(function* () {
...
): Effect.Effect<A, never, Effect.Transaction> => TxRef.get(self.ref);
...
export const changes = <A>(
	...
	Effect.acquireRelease(
		...
		(queue) =>
			Effect.transaction(TxPubSub.releaseSubscriber(self.pubsub, queue)),
	);
...
export const changesStream = <A>(
	...
	Stream.unwrap(
		Effect.map(changes(self), (sub) =>
			Stream.fromEffectRepeat(Effect.transaction(TxQueue.take(sub))),
		),
	);
