			...
			const splitLinesArray = (
				...
				for (let i = 0; i < chunk.length; i++) {
					...
					if (str.length !== 0) {
								...
								if (str.length === indexOfCR + 1) {
									...
									if (indexOfLF === indexOfCR + 1) {
										if (stringBuilder.length === 0) {
											...
											stringBuilder =
												stringBuilder + str.substring(from, indexOfCR);
											chunkBuilder.push(stringBuilder);
											stringBuilder = "";
										}
										from = indexOfCR + 2;
						...
						if (midCRLF) {
							stringBuilder =
								stringBuilder + str.substring(from, str.length - 1);
						} else {
...
> =>
	fromTransform((upstream, _scope) =>
		Effect.sync(() => {
			...
			return Effect.map(
				upstream,
				Arr.map((line) => decoder.decode(line)),
			);
		}),
	);
...
>(
	f: (
		services: ServiceMap.ServiceMap<Env>,
	) => Channel<OutElem, OutErr, OutDone, InElem, InErr, InDone, Env2>,
): Channel<OutElem, OutErr, OutDone, InElem, InErr, InDone, Env | Env2> =>
	fromTransform((upstream, scope) =>
		Effect.servicesWith((services: ServiceMap.ServiceMap<Env>) =>
			toTransform(f(services))(upstream, scope),
		),
	);
...
export const provide: {
	...
	>(
		...
		Exclude<Env, A> | R
	> =>
		ServiceMap.isServiceMap(layer)
			? provideServices(self, layer)
			: fromTransform((upstream, scope) =>
					Effect.flatMap(
						options?.local
