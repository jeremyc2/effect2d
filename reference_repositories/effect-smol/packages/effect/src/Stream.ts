	...
	<A, E, R, E2, R2>(
		...
		transformPullBracket(
			...
			Effect.fnUntraced(function* (pull, _, scope) {
				...
				let causeSink: Cause.Cause<E2> | undefined;
				let sinkDone = false;
	...
	> {
		...
		yield* Effect.gen(function* () {
			while (true) {
				...
				let passFiber: Fiber.Fiber<any> | undefined;
...
export const peel: {
	...
	Effect.fnUntraced(function* <A, E, R, A2, E2, R2>(
		...
		let cause: Cause.Cause<E | Cause.Done<void>> | undefined;
				...
				Effect.fnUntraced(function* (_upstream, _, scope) {
					...
					> {
						return step(lastOutput).pipe(
							Effect.flatMap(() =>
								!hadChunk && leftover === undefined
									? loop()
									: Queue.offer(buffer, scheduleStep),
							),
					...
					> = Queue.take(buffer).pipe(
						Effect.flatMap((arr) =>
							arr === scheduleStep ? Cause.done() : Effect.succeed(arr),
						),
					);
					...
					const catchSinkHalt = Effect.flatMap(
						([value, leftover_]: Sink.End<B, A2>) => {
							...
							if (!hadChunk && buffer.state._tag === "Done")
								return Cause.done();
...
export const updateService: {
	...
	<A, E, R, I, S>(
		...
		updateServices(self, (services) =>
			ServiceMap.add(services, service, f(ServiceMap.get(services, service))),
		),
);
...
export const toReadableStreamEffect: {
	...
	<A, E, R>(
		...
		Effect.map(Effect.services<R>(), (context) =>
...
export const toAsyncIterableEffect = <A, E, R>(
	...
	Effect.map(Effect.services<R>(), (services) =>
		toAsyncIterableWith(self, services),
	);
