 ...
 * @since 4.0.0
 */
export function decodeUnknownOption<
	S extends Schema.Top & { readonly DecodingServices: never },
>(
	schema: S,
 ...
 * @since 4.0.0
 */
export function decodeUnknownResult<
	S extends Schema.Top & { readonly DecodingServices: never },
>(
	schema: S,
): (
 ...
 * @since 4.0.0
 */
export function decodeUnknownSync<
	S extends Schema.Top & { readonly DecodingServices: never },
...
>(
	...
	asPromise(encodeUnknownEffect(schema));
 ...
 * @since 4.0.0
 */
export function encodeUnknownOption<
	S extends Schema.Top & { readonly EncodingServices: never },
>(
	schema: S,
 ...
 * @since 4.0.0
 */
export function encodeUnknownResult<
	S extends Schema.Top & { readonly EncodingServices: never },
>(
	schema: S,
): (
 ...
 * @since 4.0.0
 */
export function encodeUnknownSync<
	S extends Schema.Top & { readonly EncodingServices: never },
...
const recur = memoize((ast: AST.AST): Parser => {
	...
	if (!ast.context && !ast.encoding && !ast.checks) {
		return (ou, options) => {
			...
			return parser(
				ou,
				InternalAnnotations.resolve(ast)?.["parseOptions"] ?? options,
			);
		};
	}
	const isStructural =
