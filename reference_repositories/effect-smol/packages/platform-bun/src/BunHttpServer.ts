...
export const make = Effect.fnUntraced(function* <R extends string>(
	...
	> = [(_request, _server) => new Response("not found", { status: 404 })];
			...
			function handler(request: Request, server: BunServer<WebSocketContext>) {
				return new Promise<Response>((resolve, _reject) => {
					...
					const fiber = Fiber.runIn(
						Effect.runForkWith(ServiceMap.makeUnsafe<any>(map))(httpEffect),
						scope,
					);
...
> =>
	Layer.mergeAll(
		Layer.effect(Server.HttpServer)(
			Effect.flatMap(Config.unwrap(options).asEffect(), make),
		),
		layerHttpServices,
	);
