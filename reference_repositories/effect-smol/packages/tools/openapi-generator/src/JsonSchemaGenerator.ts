	...
	function generate(
		...
		if (Arr.isArrayNonEmpty(schemas)) {
			...
			const nonRecursives = codeDocument.references.nonRecursives.map(
				({ $ref, code }) => renderSchema($ref, code),
			);
			const recursives = Object.entries(codeDocument.references.recursives).map(
				([$ref, code]) => renderSchema($ref, code),
			);
			const codes = codeDocument.codes.map((code, i) =>
				renderSchema(nameMap[i], code),
			);
